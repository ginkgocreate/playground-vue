...
export default {
  ...
  watch: {
    value(newValue) {
      ...
      if (!newValue) {
        ...
        const datepickerEl = this.$refs.datepickerDateSelector?.$el;
  ...
  methods: {
    ...
    showDatePicker() {
       if(!this.$refs.datepickerDateSelector.isOpen) {
          this.isDatepickerOpen = true;
          if (this.format === 'yyyy/MM') {this.forciblyChangeToYMDatePicker();}
          const datepickerEl = this.$refs.datepickerDateSelector?.$el;
          datepickerEl.querySelector("input").focus();
          this.$refs.datepickerDateSelector.showCalendar();
        } else {
          this.isDatepickerOpen = false;
          this.$refs.datepickerDateSelector.close();
        }
    },
    closeDatepicker() {
      ...
      this.$emit('change');
      this.isDatepickerOpen = false;
      if(this.$refs.datepickerDateSelector){
        this.$refs.datepickerDateSelector.close();
      }
    },
    setupDatepicker() {
      this.$nextTick(() => {
        const inputElement = this.$refs.datepickerDateSelector.$el.querySelector("input");

        // 日付選択フィールドにクリックリスナーを追加
        inputElement.addEventListener('click', this.openDatepicker);

        const datepickerEl = this.$refs.datepickerDateSelector?.$el;
        const calendarElements = datepickerEl?.querySelectorAll('.vdp-datepicker__calendar');

        // カレンダーにフォーカスアウトリスナーを追加
        document.addEventListener('focusout', this.handleFocusOut.bind(this, calendarElements));
      });
    },
    openDatepicker() {
      this.isDatepickerOpen = true;
    },
    handleFocusOut(calendarElements, event) {
      const calendarElement = calendarElements?.[0];
      // カレンダー外へのフォーカス移動で閉じる処理
      if (calendarElement && !calendarElement.contains(event.relatedTarget)) {
        this.isDatepickerOpen = false;
        calendarElements[0].style.display = 'none';
      }
    },
...
$secondary-color: red;
$secondary-opposite-color: red; // 今日の色
$thirdary-color: #FFF; // 年表示の文字色

$day-primary-color: #fff; // 日付の背景色
$day-secondary-color: #e6e6e6; // 日付のボーダーカラー
$day-font-color: red; // 日付の文字色

$header-font-size: #f5f8fc;
$cell-color: #f5f8fc;

$now-selected-font-color: black; // 選択中
$now-selected-border-color: black;
$now-selected-background-color: blue;
